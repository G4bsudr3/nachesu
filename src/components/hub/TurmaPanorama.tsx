    ...
    <section className="space-y-12">
      ...
      {energias.length > 0 && (
        <div>
          ...
          <div className="flex h-12 w-full overflow-hidden rounded-full border border-perestroika-preto/15">
            {energias.map((e, i) => {
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {aggregates.observacoes.map((o: Observacao, i) => {
              ...
              return (
                <article
                  key={`${o.titulo}-${i}`}
                  className="rounded-2xl border-2 border-perestroika-preto/15 bg-perestroika-bege/50 p-5"
                >
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {aggregates.constelacoes.map((c) => (
              <article
                key={c.nome}
                className="rounded-2xl border-2 border-perestroika-preto/15 bg-perestroika-bege/50 p-6"
              >
      ...
      {(!aggregates?.constelacoes || aggregates.constelacoes.length === 0) &&
        aggregates?.top_themes && aggregates.top_themes.length > 0 && (
          <div className="rounded-3xl border-2 border-perestroika-preto/15 bg-perestroika-bege/40 p-6">
            <h3 className="mb-4 font-display text-xl uppercase">temas que estão sendo construídos</h3>
            <div className="flex flex-wrap gap-2">
              {aggregates.top_themes.map((t) => (
                <div
                  key={t.theme}
                  className="inline-flex items-center gap-2 rounded-full border border-perestroika-preto/15 bg-perestroika-bege/60 px-3 py-1.5"
                >
